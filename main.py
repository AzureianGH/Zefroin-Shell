...
from crypt import decryptpass
from crypt import encryptpass
...
if droppedFile != "":
  x,y = os.path.splitext(droppedFile)
  if y == ".zss":
    zss = open(droppedFile,'r')

loggedin = 0
tempf = open("Login.csv", "a")
tempf.close()
...
def unsupported():
  print(f"\033[1;31;40m OS: '{ostype}' does not support this feature.")
  logger.warning(f"OS: '{ostype}' does not support this feature.")
def error88():
...
def error101(dir):
  print(f"\033[1;31;40m Dir: '{dir}' does not exist! 101")
  logger.error(f"\033[1;31;40m Dir: '{dir}' does not exist! 101")
...
if isAdmin() == False:
  ...
  print("Full Access Privileges Detected, all commands are available!")
print("\033[1;33;40m Zefroin Shell 5.1 COPYRIGHT OF AZUREIAN")
#
#
#
#
  ...
  try:
    while True:
        ...
        # Display plot
              ...
              print("Login and usermake system")
            elif zinput == "whoami":
              
              print("\n~"+username+"\n")
                      
            elif "dolog" in zinput:
              x,y,z = zinput.split()
              if "/newusr" in y:
                print("No symbols please!")
                try:
                  password = input("Password >> ")
                  passck = input("Repeat Password >> ")
                except:
                  print("Unable to create Usr")
                  break
                ...
                print("/in - Login to a user : /cur - Logs out of a user")
              elif "/cur" in y:
                if "/usr" in z:
                  print(username)
              elif "/out" in y:
                ...
                error1()
            elif zinput == "codeditor":
              scriptname = input("Name of script: ")
              tempf = open(scriptname, 'a')
              tempf.close()
              earead = open(scriptname,'r')
              eatemp = open(scriptname, 'w')
              ...
              while zcodebase:
                ...
                tempf = earead.read()
                last_line = None
                codewrite = input(zstr + ": ")
                if codewrite == "zef.end":
                  zcodebase = False
                  eatemp.close()
                  earead.close()
                elif codewrite == "zef.del.line":
